...
- Default guidance that `LOW_PLUS_MEDIUM` is the committed default reviewer tier policy: `FAST + low` is the baseline reviewer path, `medium` is the standard bounded escalation tier for small-scope high-risk core logic, and `STRICT / xhigh` is exceptional.
...
- Default operating baseline: `LOW_PLUS_MEDIUM` is the committed default tier policy. Start with `FAST + low`; escalate to `medium` as the standard bounded tier for small-scope high-risk core logic, and reserve `STRICT / xhigh` for exceptional cases.
